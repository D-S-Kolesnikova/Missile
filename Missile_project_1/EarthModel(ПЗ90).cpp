...
void matr_grin_geod(const double geod_coord[], double grin_coord[]) {
	...
	double N = A_GD / sqrt(1 - E_GD * E_GD * pow(sin(geod_coord[0]), 2));
	...
	grin_coord[2] = ((1 - E_GD * E_GD) * N + geod_coord[2]) * sin(geod_coord[0]);
...
Earth_Struct	GetEarthParameters(double H, double B, double L)
{
	...
	Vector F_G(0,g_norm(B, H) + delt_g_anom(B, L, H),0);//Проекции вектора напряженности поля тяготения на норм земную СК

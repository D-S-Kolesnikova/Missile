...
void GR_GDZ (Vector GDZ, Vector& GR) {
	...
	GR.Z = ((1 - _SQR(E_GD)) * N + GDZ.Z) * sin(GDZ.X);
...
void matr_sph_grin(const double grin_coord[], double sph_coord[]) {
	...
	sph_coord[1] = atan2(grin_coord[1] , grin_coord[0]);
	sph_coord[2] = atan2((sqrt(pow(grin_coord[0], 2) + pow(grin_coord[1], 2))) , (grin_coord[2]));
}

void SF_GR(Vector GR, Vector& SF) {

	SF.X = sqrt(pow(GR.X, 2) + pow(GR.Y, 2) + pow(GR.Z, 2));
	SF.Y = atan2(GR.Y , GR.X);
	SF.Z = atan2((sqrt(pow(GR.X, 2) + pow(GR.Y, 2))) , (GR.Z));
...
{-39262968.108,1472.01984,2496.13550,2305.68074},
{-37656928.613,-1395.72710,522.57926,1219.80230},
{55417.233,-4729.43278,241.06220,-1386.75430},
...
{14954047.019,564.56372,4169.62120,3107.28151},
...
{125694.630,2887.19498,-1520.35002,-2929.00588},
...
{22635646.931,2030.13210,2786.99103,1094.12609},
...
{-72296468.140,1704.30580,2570.01705,1437.94150},
...
{-5107799.845,-2806.42940,90.03200,-437.20618},
{1152914.935,1613.29970,-1398.25374,-4728.63714},
{14079228.613,1331.61419,-1428.22398,234.32978},
{24148.390,2240.63930,2260.34015,-3797.81205},
{14981240.134,-523.57471,-2437.64320,3040.87357},
{-7746811.230,2751.33446,1231.49383,2975.90618},
{-52402113.809,490.27870,-1222.02040,299.60165},
...
{-11022424.436,-1649.80829,-755.81754,-2992.38699}
	...
	for (int i = 0; i <= 59; i++) {
		...
		Vector GR_temp, SF_temp;
		GR_temp.X =  matr_dot_mass[i][1] * 1000;
		GR_temp.Y = matr_dot_mass[i][2] * 1000;
		GR_temp.Z = matr_dot_mass[i][3] * 1000;

		SF_GR(GR_temp, SF_temp);

		matr_dot_mas_sph[i][1] = SF_temp.X;
		matr_dot_mas_sph[i][2] = SF_temp.Y;
		matr_dot_mas_sph[i][3] = SF_temp.Z;
...
Earth_Struct	GetEarthParameters(double H, double B, double L)
{
	...
	R_lambda = A_GD / W_gd;
	phi_gr0 = atan2(_SQR(B_GD) , _SQR(A_GD) * tan(B));
	...
	if (H < 0) 
	{
		g0 = g_norm(B, 0) + delt_g_anom(B, L, 0);
	}
